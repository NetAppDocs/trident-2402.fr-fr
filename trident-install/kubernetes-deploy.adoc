...
====
.<strong>Gestion des clusters Kubernetes à l'aide de la NetApp Console</strong>
[%collapsible]
====
Avec Astra Trident utilisant NetApp Console, vous pouvez mettre à niveau vers la dernière version d' Astra Trident, ajouter et gérer des classes de stockage et les connecter aux environnements de travail, et sauvegarder des volumes persistants à l'aide du Cloud Backup Service.  La console prend en charge le déploiement d' Astra Trident à l'aide de l'opérateur Trident , manuellement ou à l'aide de Helm.
